...
def main():
    ...
    df_PIX = pd.DataFrame(columns=['x', 'y', 'angle'])
    df_TFT = pd.DataFrame(columns=['x', 'y', 'angle'])
    ...
    for folder in folder_list:
        ...
        PIX_OBJ =  TargetAnalysis(n_sigma=5.5, target_type = 'pixel', ydim=0.01408, xdim=0.04224,sigma=0.0014479740312245593)
        TFT_OBJ = TargetAnalysis(n_sigma=5.5, target_type='alfa', ydim=0.0297, xdim=0.04525,sigma=0.001457145205173178)
        PIX_OBJ.load_particles(folder,'PIXEL_impacts_1', part_per_job=10000) #  n_return=1
        TFT_OBJ.load_particles(folder,'TFT_impacts', part_per_job=10000) #  n_return=1


        df_key_PIX = pd.DataFrame(columns=['x', 'y', 'angle'])
        df_key_PIX['x'] = PIX_OBJ.data['x']
        df_key_PIX['y'] = PIX_OBJ.data['y']
        df_key_PIX['angle'] = PIX_OBJ.TCCP_align_angle  
        df_PIX = pd.concat([df_PIX, df_key_PIX], ignore_index=True)

        df_key_TFT = pd.DataFrame(columns=['x', 'y', 'angle'])
        df_key_TFT['x'] = TFT_OBJ.data['x']
        df_key_TFT['y'] = TFT_OBJ.data['y']
        df_key_TFT['angle'] = TFT_OBJ.TCCP_align_angle  
    ...
    df_PIX.to_hdf(Path(path_out, f'TOTAL_DC_450_XY.h5'), key='data_PIX', format='table', mode='a',
            complevel=9, complib='blosc')
    
    df_TFT.to_hdf(Path(path_out, f'TOTAL_DC_450_XY.h5'), key='data_TFT', format='table', mode='a',
            complevel=9, complib='blosc')
