...
def load_angular_scan(folder_prefix = '', energy = 450, data_dict = None, folder_list = None, path="/afs/cern.ch/work/c/cmaccani/xsuite_sim/twocryst_sim/Condor/"):
...
def load_linear_scan(folder_prefix = '', energy = 450, data_dict = None, folder_list = None, path="/afs/cern.ch/work/c/cmaccani/xsuite_sim/twocryst_sim/Condor/"):
    data_dict = {} if data_dict is None else data_dict
    if folder_list is None:
        folder_list = ls(return_list=True)
        folder_list = [folder for folder in folder_list if folder_prefix in folder]
    for folder in folder_list:
        sig_dict = {}
        sig_key = float(folder.split('__2025')[0].split('_')[-1])
        print('Loading ', sig_key)
        if energy == 450:
            sig_dict['PIX'] =  TargetAnalysis(n_sigma=5.5, target_type = 'pixel', ydim=0.01408, xdim=0.04224,sigma=0.0014479740312245593)
            sig_dict['TFT'] = TargetAnalysis(n_sigma=5.5, target_type='alfa', ydim=0.0297, xdim=0.04525,sigma=0.001457145205173178)
        sig_dict['PIX'].load_particles(folder,'PIXEL_impacts_1', part_per_job=10000)
        sig_dict['TFT'].load_particles(folder,'TFT_impacts', part_per_job=10000)
        sig_dict['TCCP_abs'] = sig_dict['PIX'].n_TCCP_absorbed
        sig_dict['jaw_U'] = sig_dict['PIX'].TCCP_jaw_U
        sig_dict['jaw_D'] = sig_dict['PIX'].TCCP_jaw_D
        sig_dict['npart'] = sig_dict['PIX'].n_jobs*sig_dict['PIX'].part_per_job
        data_dict[sig_key] = sig_dict
        data_dict = dict(sorted(data_dict.items()))
...
def main():
    ...
    lin_scan_dict = load_linear_scan("TWOCRYST_DC_450GeV_TCCP_SCAN_AMORPH_")
    keys = list(lin_scan_dict.keys())
    ALFA_x, ALFA_y = lin_scan_dict[keys[0]]['PIX'].get_pentagonal(y_lim = lin_scan_dict[keys[0]]['PIX'].jaw_L + 200e-6)
    xlim = [np.min(ALFA_x)-1e-3, np.max(ALFA_x)+1e-3]
    ylim = [lin_scan_dict[keys[0]]['PIX'].jaw_L-3.5e-3, np.max(ALFA_y)+1e-3]

    df_PIX = pd.DataFrame(columns=['y', 'sig'])
    df_TFT = pd.DataFrame(columns=['y', 'sig'])

    for sig in lin_scan_dict.keys():

        fig_pix, ax_pix = lin_scan_dict[sig]['PIX'].plot_y(return_fig=True, set_y_dim = 36000)
        fig_tft, ax_tft = lin_scan_dict[sig]['TFT'].plot_y(return_fig=True, set_y_dim = 10000)
        
        PIX_TITLE = (f'PIXEL    '  + r'$\sigma_{TCCP}$: '  + f'{sig:.1f}'  + r'    $n_{{abs}}$: '  + f"{lin_scan_dict[sig]['PIX'].n_TCCP_absorbed / sum(lin_scan_dict[sig]['PIX'].data.y > lin_scan_dict[sig]['PIX'].jaw_L) * 100:.3f} %")
        TFT_TITLE = (f'TFT    '    + r'$\sigma_{TCCP}$: '  + f'{sig:.1f}'  + r'    $n_{{abs}}$: ' + f"{lin_scan_dict[sig]['TFT'].n_TCCP_absorbed / sum(lin_scan_dict[sig]['TFT'].data.y > lin_scan_dict[sig]['TFT'].jaw_L) * 100:.3f} %")
        ax_pix.set_title(PIX_TITLE, fontsize=14)   
        ax_tft.set_title(TFT_TITLE, fontsize=14)   

        fig_tft2D, ax_tft2D = plt.subplots(figsize=(8, 5))
        lin_scan_dict[sig]['TFT'].plot_xy_distribution(fig_tft2D, ax_tft2D, lin_scan_dict[sig]['TFT'].data['x'],  lin_scan_dict[sig]['TFT'].data['y'], bins= round( lin_scan_dict[sig]['TFT'].ydim /30e-6), xlim = xlim, ylim = ylim)
        ax_tft2D.set_title(TFT_TITLE, fontsize=14)   
        plot_support( lin_scan_dict[sig]['TFT'], ax_tft2D)
        ax_tft2D.grid(True)

        fig_pix2D, ax_pix2D = plt.subplots(figsize=(8, 5))
        lin_scan_dict[sig]['PIX'].plot_xy_distribution(fig_pix2D, ax_pix2D, lin_scan_dict[sig]['PIX'].data['x'], lin_scan_dict[sig]['PIX'].data['y'], bins= round(lin_scan_dict[sig]['PIX'].ydim /55e-6), xlim = xlim, ylim = ylim)
        ax_pix2D.set_title(PIX_TITLE, fontsize=14)   
        plot_support(lin_scan_dict[sig]['PIX'], ax_pix2D)
        ax_pix2D.grid(True)

        fig_pix.savefig(Path(path_out,f'LS_PIX_{sig}.png'), dpi=300, bbox_inches='tight')
        fig_tft.savefig(Path(path_out,f'LS_TFT_{sig}.png'), dpi=300, bbox_inches='tight')
        fig_tft2D.savefig(Path(path_out,f'LS_TFT_2D_{sig}.png'), dpi=300, bbox_inches='tight')
        fig_pix2D.savefig(Path(path_out,f'LS_PIX_2D_{sig}.png'), dpi=300, bbox_inches='tight')
        ...
        df_key_PIX = pd.DataFrame(columns=['y', 'sig'])
        df_key_PIX['y'] = lin_scan_dict[sig]['PIX'].data['y']
        df_key_PIX['sig'] = sig
        df_PIX = pd.concat([df_PIX, df_key_PIX], ignore_index=True)

        df_key_TFT = pd.DataFrame(columns=['y', 'sig'])
        df_key_TFT['y'] = lin_scan_dict[sig]['TFT'].data['y']
        df_key_TFT['sig'] = sig
        df_TFT = pd.concat([df_TFT, df_key_TFT], ignore_index=True)


    df_PIX.to_hdf(Path(path_out, f'TOTAL_LS_DC_450.h5'), key='data_PIX', format='table', mode='a',
            complevel=9, complib='blosc')
    
    df_TFT.to_hdf(Path(path_out, f'TOTAL_LS_DC_450.h5'), key='data_TFT', format='table', mode='a',
            complevel=9, complib='blosc')


    fig, ax = plt.subplots(figsize=(8, 5))
    h = ax.hist2d(df_PIX.sig, df_PIX.y, bins=[len(lin_scan_dict.keys()), 300])
    axins = inset_axes(ax, height="100%",  width="5%", loc='right', borderpad=-3)
    fig.colorbar(h[3], cax=axins, orientation='vertical', label='Count (log scale)')
    ax.set_xlabel(r'$\sigma_{TCCP}$')
    ax.set_ylabel('y [m]')   
    fig.savefig(Path(path_out,'TOTAL_LS_DC_PIX_450.png'), dpi=300, bbox_inches='tight')

    fig, ax = plt.subplots(figsize=(8, 5))
    h = ax.hist2d(df_TFT.sig, df_TFT.y, bins=[len(lin_scan_dict.keys()), 300])
    axins = inset_axes(ax, height="100%",  width="5%", loc='right', borderpad=-3)
    fig.colorbar(h[3], cax=axins, orientation='vertical', label='Count (log scale)')
    ax.set_xlabel(r'$\sigma_{TCCP}$')
    ax.set_ylabel('y [m]')   
    fig.savefig(Path(path_out,'TOTAL_LS_DC_TFT_450.png'), dpi=300, bbox_inches='tight')


    fig, ax = plt.subplots(figsize=(8, 5))
    n_cry_abs = []
    sig = []
    for key in lin_scan_dict.keys():
        n_cry_abs.append(lin_scan_dict[key]['TCCP_abs'])
        sig.append(key)
    df_abs = pd.DataFrame({'sig': sig, 'n_cry_abs': n_cry_abs})
    df_abs.to_hdf(Path(path_out, f'TOTAL_LS_DC_450.h5'), key='TCCP_abs', format='table', mode='a',
            complevel=9, complib='blosc')
    ax.plot(sig, n_cry_abs, marker='o')
    ax.set_xlabel(r'$\sigma$', fontsize=12)
    ax.set_ylabel(r'$n_{abs}$', fontsize=12)
    ax.tick_params(axis='both', which='major', labelsize=12)
    fig.savefig(Path(path_out,'TOTAL_LS_DC_TCCP_ABS_450.png'), dpi=300, bbox_inches='tight')
