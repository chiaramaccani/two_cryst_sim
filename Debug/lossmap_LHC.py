...
def main():
    ...
    if debug_2:
        ...
        file_path = './crystal_particles.txt'

        # Define the column names based on the format of your data
        #column_names = ['id', 'pc', 'L_chan', 't_I', 'xp', 't_P', 'sc', 'i_slot', 't_chord', 'drift_length']
        column_names = ['id', 'p0c_bef', 't_I_bef', 't_P_bef', 'r_bef', 'p0c', 't_I', 't_P', 'r', 'L_chan', 'pc']
        ...
        df.to_hdf(Path(path_out, f'particles_B{beam}{plane}.h5'), key='chann', format='table', mode='a',
                        complevel=9, complib='blosc')
        ...
        # Define the column names based on the format of your data
        column_names = ['id', 'pc', 'length', 'length_VI', 'length_diff', 'xp', 'xp_orig',  'r', 't_I', 'part_p0c']
        ...
        for col in df.columns:
            df[col] = df[col].str.replace(r'\w+=', '', regex=True)
        df.to_hdf(Path(path_out, f'particles_B{beam}{plane}.h5'), key='VC', format='table', mode='a',
                        complevel=9, complib='blosc')
        ...
        # Define the column names based on the format of your data
        column_names = ['id', 'pc_bef', 'pc', 'length_VI', 'p0c_bef', 'p0c']
        ...
        for col in df.columns:
            df[col] = df[col].str.replace(r'\w+=', '', regex=True)
        df.to_hdf(Path(path_out, f'particles_B{beam}{plane}.h5'), key='AM', format='table', mode='a',
                        complevel=9, complib='blosc')
        ...
        # Define the column names based on the format of your data
        column_names = ['id', 'pc_very_before', 'pc_bef', 'pc', 'channeled_length', 'length', 'L_dechan', 'L_chan', 't_I', 't_P']
        ...
        for col in df.columns:
            df[col] = df[col].str.replace(r'\w+=', '', regex=True)
        df.to_hdf(Path(path_out, f'particles_B{beam}{plane}.h5'), key='DCH', format='table', mode='a',
                        complevel=9, complib='blosc')



        file_path = './chann_transp.txt'

        # Define the column names based on the format of your data
        column_names = ['id', 'pc_bef', 'pc', 'L_chan', 'energy_loss', 'drift_length']
        ...
        for col in df.columns:
            df[col] = df[col].str.replace(r'\w+=', '', regex=True)
        df.to_hdf(Path(path_out, f'particles_B{beam}{plane}.h5'), key='ch_trans', format='table', mode='a',
                        complevel=9, complib='blosc')
        ...
        # Define the column names based on the format of your data
        column_names = ['id', 'where', 'prob_tail', 'prob_tail_c1', 'prob_tail_c2', 'prob_tail_c3', 'length', 'loglength', 'energy_loss', 'energy_loss_tail']
        ...
        for col in df.columns:
            df[col] = df[col].str.replace(r'\w+=', '', regex=True)
        df.to_hdf(Path(path_out, f'particles_B{beam}{plane}.h5'), key='loss', format='table', mode='a',
                        complevel=9, complib='blosc')
        ...
        # Define the column names based on the format of your data
        #column_names = ['pc', 'prob_factor', 'prob_tail_c1', 'prob_tail_c2', 'prob_tail_c3', 'prob_tail_c4', 'energy_loss', 'energy_loss_tail', 'Tt', 'plen',  'Tmax', 'exenergy', 'rho', 'anuc', 'zatom', 'momentum', 'energ', 'gammar', 'betar', 'bgr', 'mep']
        column_names = ['CIAO!']
        ...
        for col in df.columns:
            df[col] = df[col].str.replace(r'\w+=', '', regex=True)
        df.to_hdf(Path(path_out, f'particles_B{beam}{plane}.h5'), key='prob', format='table', mode='a',
                        complevel=9, complib='blosc')
        ...
        # Define the column names based on the format of your data
        column_names = ['id', 'only_mcs_bef', 'atom_bef', 'anuc_bef', 'rho_bef', 'exenergy_bef', 'bnref_ref', 'only_mcs', 'zatom', 'anuc', 'rho', 'exenergy', 'bnref']
        ...
        for col in df.columns:
            df[col] = df[col].str.replace(r'\w+=', '', regex=True)
        df.to_hdf(Path(path_out, f'particles_B{beam}{plane}.h5'), key='coll', format='table', mode='a',
                        complevel=9, complib='blosc')

        file_path = './coll_init.txt'

        # Define the column names based on the format of your data
        column_names = ['id', 'only_mcs', 'atom', 'anuc', 'rho', 'exenergy', 'bnref', 'eum', 'iniinit']
        ...
        for col in df.columns:
            df[col] = df[col].str.replace(r'\w+=', '', regex=True)
        df.to_hdf(Path(path_out, f'particles_B{beam}{plane}.h5'), key='coll_init', format='table', mode='a',
                        complevel=9, complib='blosc')
        
        file_path = './scatt.txt'

        # Define the column names based on the format of your data
        column_names = ["ecmsq", "xln15s", "bpp", "xintl", "bn", "cprob0", "cprob1", "cprob2", "cprob3", "cprob4", "cprob5"]
        ...
        for col in df.columns:
            df[col] = df[col].str.replace(r'\w+=', '', regex=True)
        df.to_hdf(Path(path_out, f'particles_B{beam}{plane}.h5'), key='scatt', format='table', mode='a',
                        complevel=9, complib='blosc')
